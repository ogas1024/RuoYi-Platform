...
/* ======================================================================
   预约唯一索引调整（允许取消/驳回后重新提交同一时间段）
   日期：2025-11-06
   说明：原唯一索引 uk_booking_once 为 (room_id, applicant_id, start_time, end_time)
         会阻止“取消/驳回”后再次提交同时间段。改为包含 status 字段，
         仅对相同状态的记录做唯一约束，从而允许在取消/驳回后重新提交。
====================================================================== */
-- 若已存在旧索引则删除
ALTER TABLE `tb_facility_booking` DROP INDEX `uk_booking_once`;
-- 新唯一索引：含 status
ALTER TABLE `tb_facility_booking`
    ADD UNIQUE KEY `uk_booking_once_status`(`room_id`,`applicant_id`,`start_time`,`end_time`,`status`);
...
----------------------------------------------------------------------
  日期：2025-11-06
  模块：功能房预约（Facility Booking）
  说明：新增功能房预约模块相关表，满足 30 分钟粒度预约、
        审核开关（模块内管理）、封禁仅限制申请人、房间禁用
        仅阻止新预约、允许进行中预约提前结束等需求。
======================================================================
*/
