<template>
  <el-dialog :model-value="modelValue" @update:modelValue="$emit('update:modelValue', $event)" :title="title" width="600px">
    ...
    <template #footer>
      ...
      <el-button v-if="showActions && canApprove" type="success" @click="$emit('approve', info.id)" v-hasPermi="['manage:facility:booking:audit:approve']">通过</el-button>
      <el-button v-if="showActions && canApprove" type="warning" @click="onReject" v-hasPermi="['manage:facility:booking:audit:reject']">驳回</el-button>
      <el-button v-if="showActions && canCancel" type="danger" @click="$emit('cancel', info.id)" v-hasPermi="['manage:facility:booking:cancel']">取消</el-button>
    </template>
  </el-dialog>
</template>
...
const canCancel = computed(()=> {
  ...
  try { return new Date(info.value?.startTime) > new Date() } catch(e){ return false }
})
...
async function onReject(){
  const { value, action } = await ElMessageBox.prompt('请输入驳回理由','驳回',{ confirmButtonText:'确定', cancelButtonText:'取消', inputPlaceholder:'理由', inputType:'textarea' }).catch(()=>({ action:'cancel' }))
