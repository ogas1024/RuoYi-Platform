...
function renderItem(params, api){
  ...
  const height = 20
  const rectShape = echarts.graphic.clipRectByRect({ x: start[0], y: params.coordSys.y + (params.coordSys.height - height)/2, width: end[0]-start[0], height }, { x: params.coordSys.x, y: params.coordSys.y, width: params.coordSys.width, height: params.coordSys.height })
...
function toSeriesData(list){
  return (list||[]).map(s => ({ value: [new Date(s.start).getTime(), new Date(s.end).getTime(), 0, s.status], meta: { id: s.id, start: s.start, end: s.end, status: s.status } }))
}

function toTs(v){ if(!v) return null; const t = (v instanceof Date)? v.getTime(): new Date(v).getTime(); return isNaN(t)? null : t }
...
function init(){
  ...
  chart.on('click', (params) => { const id = params?.data?.meta?.id; if (id) emit('segment-click', id) })
}
...
onMounted(()=>{ init(); update(); window.addEventListener('resize', resize) })
