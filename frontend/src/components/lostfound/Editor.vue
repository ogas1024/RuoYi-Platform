<template>
  <el-form :model="data" label-width="100px" style="max-width: 760px">
    ...
    <el-form-item label="标题"><el-input v-model="data.title" placeholder="2~50 字" maxlength="50" show-word-limit/></el-form-item>
    <el-form-item label="正文"><el-input v-model="data.content" type="textarea" :rows="6" placeholder="尽量描述清楚，支持换行"/></el-form-item>
    <el-form-item label="联系方式"><el-input v-model="data.contactInfo" placeholder="电话/微信等（≤50字，可选）" maxlength="50" show-word-limit/></el-form-item>
    <el-form-item label="地点"><el-input v-model="data.location" placeholder="可选"/></el-form-item>
...
const data = reactive({ type: 'lost', title:'', content:'', contactInfo:'', location:'', lostTime:'', images: [], ...(props.modelValue||{}) })
...
watch(data, (val) => { emit('update:modelValue', JSON.parse(JSON.stringify(val))) }, { deep: true })
...
watch(() => data.images, (imgs) => {
  fileList.value = (imgs || []).map((img, i) => ({ name: img.url || `img-${i}` , url: img.url, status: 'success', uid: img.uid || `${i}-${img.url}` }))
...
const onPreview = (file) => { previewUrl.value = file.url; previewOpen.value = true }

const ossUpload = async ({ file, onSuccess, onError }) => {
  if ((data.images||[]).length >= props.maxImages) { ElMessage.error('最多 9 张'); return }
  if (!(file && file.type && file.type.startsWith('image/'))) { ElMessage.error('仅支持图片类型'); return }
  const limit = (props.maxSizeMb || 2) * 1024 * 1024
  if (file.size > limit) { ElMessage.error(`图片大小不能超过 ${props.maxSizeMb}MB`); return }
...
<style scoped>
.img-uploader .tip { color: #909399; font-size: 12px; margin-top: 6px; }
