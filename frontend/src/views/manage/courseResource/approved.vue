  ...
  <div class="app-container">
    ...
    <el-form :inline="true" :model="queryParams" class="mb8">
      ...
      <el-form-item label="关键字">
        <el-input v-model="queryParams.keyword" placeholder="资源名/简介/上传者" style="width: 220px"
    ...
    <el-table v-loading="loading" border stripe :data="list">
      ...
      <el-table-column label="操作" fixed="right" width="460">
        <template #default="scope">
          ...
          <el-button link :type="scope.row.isBest===1?'info':'success'" icon="Star" @click="toggleBest(scope.row)"
                     v-hasPermi="['manage:courseResource:best']">{{ scope.row.isBest === 1 ? '取消最佳' : '设为最佳' }}
          </el-button>
          <el-button link type="warning" icon="Bottom" @click="openOffline(scope.row)"
                     v-hasPermi="['manage:courseResource:offline']">下架
          </el-button>
          <el-button link type="primary" icon="Edit" @click="editRow(scope.row)"
                     v-hasPermi="['manage:courseResource:edit']">编辑
          </el-button>
          <el-button link type="danger" icon="Delete" @click="delRow(scope.row)"
                     v-hasPermi="['manage:courseResource:remove']">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
...
import {
  ...
  unsetBestResource,
  getResource
} from '@/api/manage/courseResource'
...
const editRow = (row) => {
  Object.assign(form, {id: row.id, resourceName: row.resourceName, description: row.description});
  open.value = true
}
const submitForm = () => {
