    ...
    <el-table :data="list" v-loading="loading" border>
      ...
      <el-table-column label="操作" width="260">
        <template #default="{ row }">
          <el-button size="small" type="primary" text @click="openDetail(row)" v-hasPermi="['manage:library:get']">
            查看
          </el-button>
          <el-button size="small" type="success" text @click="approve(row)" v-hasPermi="['manage:library:approve']">
            通过
          </el-button>
          <el-button size="small" type="warning" text @click="openReject(row)" v-hasPermi="['manage:library:reject']">
            驳回
          </el-button>
        </template>
      </el-table-column>
