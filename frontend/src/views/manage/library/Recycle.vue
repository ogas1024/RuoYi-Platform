    ...
    <el-table :data="list" v-loading="loading" border>
      ...
      <el-table-column label="操作" width="320">
        <template #default="{ row }">
          ...
          <el-button size="small" type="primary" text @click="online(row)" v-hasPermi="['manage:library:online']">
            上架(待审)
          </el-button>
          <el-button size="small" type="danger" text @click="remove(row)" v-hasPermi="['manage:library:remove']">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
...
const resetQuery = () => {
  queryParams.value = {pageNum: 1, pageSize: 10, status: null};
  getList()
}
const online = async (row) => {
  await onlineLibrary(row.id);
  proxy && proxy.$modal && proxy.$modal.msgSuccess('已提交上架审核');
  getList()
}
const remove = async (row) => {
  await delLibrary(row.id);
  proxy && proxy.$modal && proxy.$modal.msgSuccess('删除成功');
  getList()
}
