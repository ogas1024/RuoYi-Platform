    ...
    <el-table :data="list" v-loading="loading" border>
      ...
      <el-table-column label="操作" width="280">
        <template #default="{ row }">
          ...
          <el-button size="small" text type="success" @click="approve(row)" v-hasPermi="['manage:lostfound:audit:approve']">通过</el-button>
          <el-button size="small" text type="danger" @click="openReject(row)" v-hasPermi="['manage:lostfound:audit:reject']">驳回</el-button>
        </template>
      </el-table-column>
    </el-table>
...
const reason = ref('')
const openReject = (row) => { currentId.value = row.id; reason.value=''; rejectOpen.value = true }
