<template>
  <div class="app-container">
    ...
    <el-table :data="list" v-loading="loading" border>
      ...
      <el-table-column label="已解决" width="90"><template #default="{ row }"><el-tag :type="row.solvedFlag==1?'':'warning'">{{ row.solvedFlag==1?'是':'否' }}</el-tag></template></el-table-column>
      <el-table-column label="操作" width="360">
        <template #default="{ row }">
          <el-button size="small" text type="primary" @click="openDetail(row)" v-hasPermi="['manage:lostfound:get']">详情</el-button>
          <el-button size="small" text @click="openEdit(row)" v-hasPermi="['manage:lostfound:edit']">编辑</el-button>
          <el-button size="small" text type="warning" @click="openOffline(row)" v-hasPermi="['manage:lostfound:offline']">下架</el-button>
    ...
    <el-dialog v-model="detailOpen" title="详情" width="720px">
      <div v-if="detail">
        ...
        <div class="images" v-if="images.length">
          <el-image v-for="img in images" :key="img.id" :src="img.url" :preview-src-list="images.map(i=>i.url)" style="width:120px;height:120px;margin:4px;object-fit:cover"/>
        </div>
      </div>
    </el-dialog>
...
import { listLostFound, getLostFound, offlineLostFound, removeLostFound, setSolvedLostFound, updateLostFound } from '@/api/manage/lostfound'
...
const resetQuery = () => { q.value = { type:'', keyword:'' }; page.pageNum = 1; getList() }
...
const reason = ref('')
const openOffline = (row) => { currentId.value = row.id; reason.value=''; offlineOpen.value = true }
