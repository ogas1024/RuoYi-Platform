...
function loadMeta() {
  listRole({pageNum: 1, pageSize: 100}).then(r => {
    roleOptions.value = r.rows || r.data || []
  })
  getDeptTree().then(r => {
    deptTree.value = r.data || []
  })
  listPost({pageNum: 1, pageSize: 100}).then(r => {
    postOptions.value = r.rows || r.data || []
  })
}
...
function addUrlAttachment() {
  ElMessageBox.prompt('请输入文件URL', '添加附件链接', {
    inputPattern: /https?:\/\//i,
    inputErrorMessage: 'URL 必须以 http(s) 开头'
  })
      .then(({value}) => {
        attachments.push({
...
function handleFileChange(e) {
  ...
  uploadOss(fd, {dir: 'notice', publicUrl: true}).then(res => {
    ...
    attachments.push({
      fileName: file.name,
      fileUrl: data.url || data.data?.url || '',
      fileType: ext,
      fileSize: file.size,
      sort: attachments.length + 1
    })
...
function onSubmit() {
  ...
  if (form.visibleAll === 0) {
    ...
    if (!scopes.length) {
      ElMessage.error('可见范围为自定义时，至少选择一个角色/部门/岗位');
      return
    }
  ...
  api(payload).then(async r => {
    ...
    try {
      await ElMessageBox.confirm('保存成功，是否立即发布？', '发布提示', {
        type: 'info',
