...
function addItem(t){ if (form.items.length>0) return; form.items.push({ title:'', type:t, required:1, options:[] }) }
...
function baseCheck(){
  ...
  for (const op of it.options) { if (!op.label || !op.label.trim()) return ElMessage.error('选项文本不能为空') }
  return true
}

function save(){ if (baseCheck()!==true) return; const payload = {
  id: form.id || undefined, title: form.title, deadline: form.deadline || null, visibleAll: form.visibleAll,
  items: form.items.map((x, idx) => ({ title: x.title, type: x.type, required: x.required, sortNo: idx,
...
function publish(){ if (baseCheck()!==true) return;
  if (!form.id) {
    const payload = {
    ...
    addVote(payload).then(()=>{ ElMessage.success('已发布'); router.push({ name:'ManageVoteIndex' }) })
  } else { publishVote(form.id).then(()=>{ ElMessage.success('已发布'); router.push({ name:'ManageVoteIndex' }) }) }
}
...
onMounted(()=>{
  ...
  if (id) {
    getVote(id).then(res=>{
      const d = res.data
      form.id = d.id; form.title = d.title; form.deadline = d.deadline; form.visibleAll = d.visibleAll
      const first = (d.items||[])[0]
      if (first) {
        form.items = [{ id: first.id, title: first.title, type: first.type, required: first.required,
