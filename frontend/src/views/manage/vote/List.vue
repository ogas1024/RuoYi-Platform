          ...
          <template #default="scope">
            ...
            <template v-if="scope.row.status===0">
              <el-button size="small" type="primary"
                         @click="$router.push({ name:'ManageVoteNew', query:{ id: scope.row.id } })"
...
import {
  ...
  archiveVote,
  extendVote,
  listVoteSubmittedUsers,
  getVoteUserAnswers,
  pinVote,
  publishVote
} from '@/api/manage/vote'
...
function confirmExtend() {
  ...
  extendVote(extendRow.value.id, extendDeadline.value).then(() => {
...
function renderCharts() {
  ...
  for (const it of detail.value.items) {
    if (it.options && it.options.length > 0) renderChartForItem(it)
  }
}
