  ...
  <div class="app-container" v-loading="loading">
    ...
    <div class="detail">
      ...
      <div class="info">
        ...
        <div class="meta">ISBN：{{ book.isbn13 }} · 作者：{{ book.author }} · 出版社：{{ book.publisher || '-' }} ·
          年：{{ book.publishYear || '-' }}
        </div>
        <div class="meta">上传者：{{ book.uploaderName || '-' }} · 上传时间：{{ book.createTime || '-' }}</div>
        <div class="actions">
          <el-button type="primary" @click="download()">直接下载</el-button>
          <el-button :type="favorite?'warning':'default'" @click="toggleFav">{{
              favorite ? '取消收藏' : '收藏'
            }}
          </el-button>
        </div>
        <div class="assets" v-if="assets.length">
          ...
          <el-space>
            <el-button v-for="a in fileAssets" :key="a.id" size="small" @click="download(a.id)">
              {{ a.format?.toUpperCase() || '文件' }}
            </el-button>
            <el-button v-for="a in linkAssets" :key="'l'+a.id" size="small" type="success" @click="openLink(a.linkUrl)">
              外链
            </el-button>
          </el-space>
        </div>
