...
const submit = () => {
  formRef.value.validate(async valid => {
    ...
    const payload = { ...form.value, assets: combined.map((a, idx) => ({
...
const reset = () => {
  form.value = { isbn13: '', title: '', author: '', publisher: '', publish_year: null, language: '', keywords: '', summary: '' }
  pdfAsset.value = null
  extraAssets.value = []
...
const removePdf = () => { pdfAsset.value = null }
...
const pushLinkExtra = () => {
  ...
  extraAssets.value.push({ id: Date.now(), assetType: '1', format: '', fileUrl: '', fileSize: 0, fileHash: '', linkUrl: fixed })
  extra.value.link_url = ''
}
