  ...
  <div class="portal-container">
    ...
    <el-table :data="list" v-loading="loading">
      ...
      <el-table-column label="操作" width="220">
        <template #default="scope">
          <el-button size="small" type="primary" v-if="!ended(scope.row)" @click="modify(scope.row)">修改投票</el-button>
...
function load(){
  ...
  myVote({ pageNum: pageNum.value, pageSize: pageSize.value }).then(res=>{
    ...
    ids.forEach(id=>{
      ...
      }).catch(()=>{ endMap.value[id] = false })
    })
  }).finally(()=>loading.value=false)
}

function ended(row){ const val = endMap.value[row.surveyId]; return !!val }
function modify(row){ router.push({ name:'PortalVoteFill', query:{ id: row.surveyId } }) }
function view(row){ router.push({ name:'PortalVoteFill', query:{ id: row.surveyId } }) }
onMounted(load)
</script>
