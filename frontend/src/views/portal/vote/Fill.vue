<template>
  <div class="portal-container" v-loading="loading">
    ...
    <el-form v-if="detail" label-width="100px">
      <div v-for="(it,idx) in detail.items" :key="it.id"
           style="margin:10px 0; padding:10px; border:1px dashed var(--el-border-color); border-radius:6px;">
...
function load() {
  ...
  getVote(id).then(res => {
    ...
    nextTick(() => {
      if (showResult.value) setTimeout(renderCharts, 0)
    })
...
function submit() {
  ...
  const payload = Object.values(answers).map(x => ({
    itemId: x.itemId,
    optionIds: Array.isArray(x.optionIds) ? x.optionIds : (typeof x.optionIds === 'number' ? [x.optionIds] : [])
  }))
  const wasSubmitted = hasSubmitted()
  submitVote(id, payload).then(() => {
...
function renderCharts() {
  ...
  for (const it of detail.value.items) {
    if ((it.type === 2 || it.type === 3) && it.options && it.options.length > 0) renderChartForItem(it)
  }
}
