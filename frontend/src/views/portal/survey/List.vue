  ...
  <div class="portal-container">
    ...
    <el-table :data="list" v-loading="loading">
      ...
      <el-table-column label="操作" width="180">
        <template #default="scope">
          <template v-if="!(scope.row.status===2 || isExpired(scope.row))">
            <el-button size="small" type="primary" @click="fill(scope.row)">填写</el-button>
          </template>
          <template v-else>
            <el-button size="small" @click="fill(scope.row)">查看结果</el-button>
          </template>
        </template>
      </el-table-column>
    </el-table>
