    ...
    <el-tabs v-model="activeTab" @tab-change="onTabChange">
      <el-tab-pane label="资源排行" name="resource">
        <div class="toolbar">
          ...
          <el-select v-if="scope==='major'" v-model="majorId" clearable placeholder="选择专业"
          ...
          <template v-if="scope==='course'">
            <el-select v-model="majorId" clearable placeholder="选择专业" style="margin-left: 8px; width: 220px"
                       @change="loadCourses">
              <el-option v-for="m in majors" :key="m.id" :label="m.majorName" :value="m.id"/>
            </el-select>
        ...
        <el-table v-loading="loading" border stripe :data="list" size="small">
          ...
          <el-table-column label="资源名称" min-width="240" show-overflow-tooltip>
            <template #default="scope">
              <el-tag v-if="scope.row.isBest===1" type="warning" size="small" effect="dark" style="margin-right:6px">
                最佳
              </el-tag>
      ...
      <el-tab-pane label="用户积分" name="score">
        <div class="toolbar">
          ...
          <el-select v-model="scoreMajorId" clearable placeholder="选择专业（不选=全站）"
          ...
          <el-table-column label="代表作" min-width="420">
            <template #default="scope">
              <span v-if="scope.row.repWorks && scope.row.repWorks.length">
                <el-space wrap>
                  <el-tooltip v-for="w in scope.row.repWorks.slice(0,5)" :key="w.id"
                              :content="`${w.courseName || '-'} | ${w.publishTime || '-'}`" placement="top">
                    <el-tag size="small" :type="w.isBest===1 ? 'warning' : 'info'" effect="plain" style="cursor:pointer"
                            @click="openDetailById(w.id)">
    ...
    <el-dialog v-model="detailVisible" title="资源详情" width="680px">
      <el-descriptions :column="2" border size="small">
        ...
        <el-descriptions-item label="上传时间">{{
            (detail.publishTime || detail.createTime) || '-'
          }}
    ...
    <el-drawer v-model="worksDrawerVisible" :title="`代表作 - ${worksDrawerUser?.username || ''}`" size="60%">
      <div class="toolbar" style="margin-bottom:8px">
        <el-form :inline="true" :model="worksQuery">
          ...
          <el-form-item label="课程">
            <el-select v-model="worksQuery.courseId" placeholder="课程" clearable :disabled="worksCourses.length === 0"
                       style="width:220px">
      ...
      <el-table :data="worksDrawerList" border stripe size="small">
        ...
        <el-table-column label="资源名称" min-width="260" show-overflow-tooltip>
          <template #default="scope">
            <el-tag v-if="scope.row.isBest===1" type="warning" size="small" effect="dark" style="margin-right:4px">
              最佳
            </el-tag>
            <span>{{ scope.row.resourceName }}</span>
          </template>
...
const openAllWorks = async (row) => {
  ...
  try {
    ...
    worksDrawerFullList.value = sorted.map(it => ({
      id: it.id,
