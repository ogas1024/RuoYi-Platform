<template>
  <div class="app-container">
    ...
    <el-dialog v-model="open" title="详情" width="720px">
      <div v-if="detail">
        <h3>{{ detail.title }}</h3>
        <div class="meta">类型：{{ detail.type==='lost'?'丢失':'捡到' }} · 地点：{{ detail.location || '-' }} · 时间：{{ detail.lostTime || '-' }}</div>
...
const handleQuery = () => { q.value.pageNum = 1; getList() }
const resetQuery = () => { q.value = { pageNum:1, pageSize:10, type:'', keyword:'', solvedFlag: includeSolved.value? undefined:0 }; getList() }
