<template>
  <div class="app-container">
    <div class="toolbar">
      <el-button type="primary" icon="Plus" @click="goCreate">发布事物</el-button>
    </div>
    ...
    <el-table :data="list" v-loading="loading" border>
      ...
      <el-table-column label="操作" width="260">
        <template #default="{ row }">
          <el-button size="small" text type="primary" @click="openEdit(row)" :disabled="row.solvedFlag==1">编辑
          </el-button>
          <el-button size="small" text type="success" @click="solve(row)"
                     :disabled="row.solvedFlag==1 || row.status!==2">标记已解决
          </el-button>
...
import {
  myLostFoundPortal,
  removeLostFoundPortal,
  solveLostFoundPortal,
  getLostFoundPortal,
  updateLostFoundPortal
} from '@/api/portal/lostfound'
...
const resetQuery = () => {
  q.value = {pageNum: 1, pageSize: 10, status: '', solvedFlag: ''};
  getList()
}
...
const solve = async (row) => {
  await solveLostFoundPortal(row.id);
