...
重要前提
- 门户（普通用户界面）统一路由前缀为 `/portal`，课程资源分享模块入口为 `/portal/course-resource`，不依赖后台菜单；写操作仍走 `/manage` 接口并受按钮权限控制。
...
- 按钮
  ...
  - 设为最佳/取消最佳：manage:courseResource:best
...
- 菜单（用于审核 `status=0` 待审资源）
  ...
  - 权限字符：manage:courseResource:list
 - 按钮
  - 下载：manage:courseResource:download（用于审核时查看资源内容）
...
- 菜单（用于筛选 `status in (2,3)`，支持重新上架/硬删除）
  ...
  - 权限字符：manage:courseResource:list
 - 按钮
  - 下载：manage:courseResource:download（需要复核内容时使用）
  - 重新上架（进入待审）：manage:courseResource:online
  - 强制删除（硬删，仅删库）：manage:courseResource:remove

提示：管理端下载接口为受保护路由（/manage/courseResource/{id}/download），前端使用 window.open 时无法自动携带请求头，已改为在 URL 上追加 `?token=<当前登录token>` 进行认证，并在服务端进行下载计数与 302 跳转。
...
七、验证流程（最小闭环）
- 用 user 角色：在门户 /portal/course-resource 上传（有按钮权限），进入“我上传的”查看状态
