...
- 阅读端：
  - 列表分页（置顶优先），关键词搜索（标题）
  - 展示已读/未读状态；进入详情自动且幂等记录阅读回执（同一用户仅首次计数）
  - 筛选：状态（含“全部=-1”）、是否包含过期、已读/未读
  - 排序：按发布时间/更新时间/过期时间（升序/降序），置顶始终优先
  - 详情展示附件并可下载
- 统计（MVP）：
  - 阅读计数（打开详情即计数）
...
- 已过期的判定：`now() > expire_time` 视为“过期”，列表默认不显示；可选 `includeExpired=true` 显示；详情可访问并显示“已过期”。
- 已发布可直接编辑（允许小修改）：更新 `update_time` 且 `edit_count+1`。
 - 撤回会自动取消置顶（`pinned=0, pinned_time=null`）。
...
- 编辑附件（MVP）：覆盖式保存（清空后重建）。若需“归档不删除附件”，可将“删除附件”改为软删（`del_flag=2`）并在查询中过滤 `del_flag='0'`（可选增强）。
- 附件白名单与大小限制：pdf/docx/xlsx/png/jpg/zip；单文件≤20MB（与网关/OSS一致）。

## 7. 接口草案（对齐 RuoYi 风格）
- GET `/manage/notice/list` 权限 `manage:notice:list`
  - 入参：分页、`keyword`、`status?`（-1=全部）、`pinned?`、`includeExpired?`（bool，默认false）、`read?`（true/false）、`orderBy?`（publishTime|updateTime|expireTime）、`orderDir?`（asc|desc）
  - 出参：含每条记录 `read`（当前用户是否已读）、`expired`（是否过期）、`pinned`、`publishTime`、`updateTime`、`expireTime`、`editCount`、`readCount`
...
- 置顶优先排序：`pinned DESC, pinned_time DESC, {orderBy} {orderDir}`（默认 `publish_time DESC`）
- 索引：
...
- 非管理员“全部(-1)”视角：仅返回“所有已发布 + 本人草稿/撤回”。
