...
## 界面信息架构（门户）
- 顶栏（Banner）
  - 左：站点 Logo（点击回到门户首页 `/portal/course-resource`）
  - 中：面包屑（课程资源 / 专业 / 课程），按路由上下文动态展示
  - 右：
    - “管理后台”按钮（仅对 admin/super_admin/major_lead/librarian 可见，跳转 `/index`）
    - 用户头像下拉：个人信息、退出登录
- 左侧导航（层叠）
  - 课程资源分享
    - 资源首页（/portal/course-resource）
    - 我上传的（/portal/course-resource/my）
    - 排行榜（/portal/course-resource/top）
      - 资源排行：支持 全站/按专业/按课程 + 天数筛选；名称列显示“最佳”徽标；下载跳转前先计数
      - 用户积分：展示用户积分排名；每行展示“代表作Top5”（按下载数降序）；点击用户名打开抽屉查看该用户“全部代表作”，支持筛选与排序
  - 数字图书馆（预留）
...
### 新增：积分与排行榜（本次草案）
- `tb_cr_user_score`（CR 专属）用户积分聚合：`id, user_id, username, major_id(0=全站), total_score, approve_count, best_count, 审计, del_flag`
- `tb_cr_user_score_log`（CR 专属）用户积分流水：`id, user_id, username, major_id, resource_id, event_type(APPROVE/BEST), delta, remark, 审计, del_flag`

业务规则（防刷）：
- 上传资源“第一次审核通过”加少量积分（默认 +5）；资源被“第一次设置为最佳”加分（默认 +10）。
- 编辑后再次提交、再次审核通过不重复加分；取消“最佳”不扣分；再次设置为最佳不重复加分。
- 以 `(user_id, resource_id, event_type)` 在流水表唯一约束实现“只发一次”。
- 聚合表维护两类行：`major_id=资源专业` 与 `major_id=0(全站)`，用于“按专业排行榜”与“全站排行榜”（专业名称通过与 `tb_major` 关联获取）。
...
- 后台管理（按钮权限控制）：
  ...
  - 排行榜（新增）：
    - 门户只读：GET `/portal/score/rank`（Query：`majorId?`，0/缺省=全站）
    - 后台查询：GET `/manage/score/user/rank`（权限：`manage:score:list`）

## 用户流程（门户）
- 学生
  1) 登录 → 进入 `/portal/course-resource`（专业卡片）
  2) 选择专业 → `/portal/course-resource/course`（课程卡片）
  3) 选择课程 → `/portal/course-resource/list`（资源卡片列表，显示已通过，最佳资源带“最佳”徽标并置顶）
  4) 分享资源（需权限）：选择“OSS 上传或外链”、填写名称与简介 → 提交审核（进入待审）
  5) “我上传的”管理：查看所有状态、编辑（回到待审）、删除（待审/驳回/下架）、下架、重新上架（回到待审）
  6) 排行榜：进入“课程资源分享 → 排行榜”
     - 资源排行：选择 scope（全站/专业/课程）与天数；可直接下载（名称列显示“最佳”徽标）
     - 用户积分：可筛选专业（或全站），按积分从高到低展示；每行显示代表作Top5；点击用户名打开抽屉查看该用户全部代表作，支持关键词/课程/类型/仅最佳/排序（按下载或时间）
- 专业负责人（后台）
...
- 硬删除：仅删数据库记录，不删 OSS 对象
 ...
 - 积分：
   - 发放仅一次，基于流水唯一约束；数据修复可通过重算任务（后续）按流水回放
   - 若资源变更专业（非常规操作），历史积分不回溯；按首次发放时的专业维度入账
