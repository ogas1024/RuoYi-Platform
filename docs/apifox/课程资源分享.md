...
- 断言：HTTP 302 跳转（Apifox 显示重定向），随后再次查询详情/列表，`downloadCount` 递增；若未携带 Authorization 头返回 401
...
{ "reason": "内容更新/版权问题/其它" }
```
- 断言：`code == 200`，状态变为 3，后台“回收站/已驳回/已下架”中 `auditReason` 可见
...
11B) 回收站详情查看
- 页面：后台 → 课程资源分享 → 回收站
- 操作：点击“查看”，在详情对话框中应高亮显示“原因：auditReason”（状态=已驳回/已下架）
