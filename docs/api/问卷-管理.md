...
### AI 汇总报告（新增）
- 方法：POST
- 路径：`/manage/survey/{id}/ai-summary`
- 权限：`manage:survey:summary`
- 请求体：
```json
{ "extraPrompt": "（可选）补充分析目标或输出格式要求，例如以 Markdown 输出表格" }
```
- 返回：
```json
{ "code":200, "msg":"成功", "data": { "text": "...AI 生成的中文汇总报告..." } }
```
- 说明：后端通过环境变量 `ZAI_API_KEY` 调用智普 `glm-4.6` 完成汇总；若未配置或调用失败，返回错误信息。
