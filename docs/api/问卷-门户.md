...
### 详情（含“我的作答”，仅发布态；投票结束后附带结果）
- 方法：GET
- 路径：`/portal/survey/{id}`
- 返回：投票未结束时不包含选项票数；当“截止时间已过”（或归档）时，返回的单/多选题选项对象将附带 `voteCount` 字段，用于前端展示柱状图。
```json
{
  "code": 200,
  "data": {
    ...
    "items": [
      ...
      { "id":12, "title":"宿舍是否分配完成？", "type":2,
        "options":[
          {"id":121,"label":"是","voteCount":20},
          {"id":122,"label":"否","voteCount":5}
        ]
      }
    ],
