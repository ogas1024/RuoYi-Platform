...
- 概览（注意：本文档为旧版总览，建议优先阅读分册文档：docs/api/课程资源-门户.md 与 docs/api/课程资源-管理.md）
- 架构：前台门户（/portal 查询 + “我的资源”自助增删改/上下架）+ 后台管理（/manage 增删改审）。
- 业务：专业 → 课程 → 资源；资源支持压缩包（OSS）或外链；创建/编辑均进入审核流转。
- 角色：普通用户（user）、专业负责人（major_lead）、管理员（admin/super_admin）。
- 重要说明：门户侧 /portal 下的“我的资源”写操作仅需登录；后台管理 /manage 的写操作需对应 `manage:*` 按钮级权限（详见“权限与鉴权”）。
...
- PUT `/manage/courseResource/{id}/reject` 权限：`manage:courseResource:reject` Body：`{ reason }`（必填）
- PUT `/manage/courseResource/{id}/offline` 权限：`manage:courseResource:offline` Body：`{ reason }`（必填）
...
- `bestBy`：标记人
- `bestTime`：标记时间
- `auditReason`：驳回/下架原因（当 status=2/3 时建议前端详情页高亮显示）
...
- 前后端对齐：门户仅读，写操作统一走 `/manage` 并按按钮权限控制，普通用户需要写权限方可上传。
 - 下载：
   - 门户下载 `/portal/resource/{id}/download` 需登录（携带 Authorization 头），计数后 302 跳转；
   - 管理端下载 `/manage/courseResource/{id}/download` 需对应按钮权限；若使用 `window.open`，可追加 `?token=<Admin-Token>` 完成认证。
