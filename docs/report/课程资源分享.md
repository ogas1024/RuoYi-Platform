...
```mermaid
erDiagram
  TB_COURSE_RESOURCE ||--o{ TB_CR_USER_SCORE_LOG : 来源资源
  TB_CR_USER_SCORE ||--o{ TB_CR_USER_SCORE_LOG : 聚合自流水
  TB_CR_USER_SCORE }o--|| TB_MAJOR : 维度(专业)

  TB_CR_USER_SCORE {
    BIGINT id PK
    BIGINT user_id
    VARCHAR username
    BIGINT major_id // 0=全站
    INT total_score
    INT approve_count
    INT best_count
    AUDIT audit_fields
  ...
  TB_CR_USER_SCORE_LOG {
    ...
    BIGINT user_id
    VARCHAR username
    BIGINT major_id // 0=全站也记录对应专业ID
    BIGINT resource_id
    VARCHAR event_type // APPROVE|BEST
    INT delta // +5 | +10
    AUDIT audit_fields
    UK(user_id, resource_id, event_type)
  }
```
...
sequenceDiagram
  ...
  participant S as ScoreServiceImpl
  participant DB as MySQL

  A->>API: PUT /manage/courseResource/{id}/approve
  API->>DB: 资源状态置为 已通过(1)
  API->>S: awardApprove(resource, operator)
  S->>DB: INSERT IGNORE tb_cr_user_score_log(event=APPROVE, delta=5)
  alt 首次发放
    S->>DB: UPSERT tb_cr_user_score(user_id, major_id=资源专业, +5, approve_count+1)
    S->>DB: UPSERT tb_cr_user_score(user_id, major_id=0, +5, approve_count+1)
  else 重复发放
    Note right of S: 已存在同(user,resource,event)流水，忽略
  end

  A->>API: PUT /manage/courseResource/{id}/best
  API->>DB: 写入/更新 tb_course_resource_best
  API->>S: awardBest(resource, operator)
  S->>DB: INSERT IGNORE tb_cr_user_score_log(event=BEST, delta=10)
  alt 首次发放
    S->>DB: UPSERT tb_cr_user_score(user_id, major_id=资源专业, +10, best_count+1)
    S->>DB: UPSERT tb_cr_user_score(user_id, major_id=0, +10, best_count+1)
  else 重复发放
    Note right of S: 取消再设为最佳不重复加分
  end
```
...
sequenceDiagram
  ...
  U->>P: 切换到“用户积分”Tab，选择专业
  P->>API: GET /portal/score/rank?majorId=0|专业ID&pageNum=1&pageSize=20
  API->>DB: SELECT s.*, m.major_name, ROW_NUMBER() OVER(ORDER BY total_score DESC) AS rank
  API-->>P: { rows:[{ rank, userId, username, majorId, majorName, totalScore, approveCount, bestCount }], total }
  P-->>U: 表格展示（全站/按专业）
```
